					...
					for ( int i = 0; i < extend_filter; i++ )
						for ( int j = 0; j < extend_filter; j++ )
							for ( int z = 0; z < in.size.z; z++ )
							{
								...
								mul++;
								string strv = FloatToByBinaryByAlgo(v).substr(0,LEN)+FloatToByBinaryByAlgo(f).substr(0,LEN);
								if(M[strv]){
									sum += m[strv];
									hit++;
				...
				for ( int z = 0; z < in.size.z; z++ )
				{
					...
					for ( int i = rn.min_x; i <= rn.max_x; i++ )
					{
						...
						for ( int j = rn.min_y; j <= rn.max_y; j++ )
						{
							...
							{
								float w_applied = filters[k].get( x - minx, y - miny, z );

								mul++;
								string strv = FloatToByBinaryByAlgo(w_applied).substr(0,LEN)+FloatToByBinaryByAlgo(grad_next_layer( i, j, k )).substr(0,LEN);
								if(M[strv]){
									sum_error += m[strv];
									hit++;
									// printf("b");
								...
								//printf("%.32f %.32f \n",w_applied,grad_next_layer( i, j, k ));
								//printf("%.32f %.32f %.32f\n",w_applied,grad_next_layer( i, j, k ),w_applied * grad_next_layer( i, j, k ));
								#ifdef Times
									fprintf(fp,"%.32f %.32f %.32f\n",w_applied,grad_next_layer( i, j, k ),w_applied * grad_next_layer( i, j, k ));
								#endif 

								string strv1 = FloatToByBinaryByAlgo(in( x, y, z )).substr(0,LEN)+FloatToByBinaryByAlgo(grad_next_layer( i, j, k )).substr(0,LEN);
								mul++;
								if(M[strv1]){
									filter_grads[k].get( x - minx, y - miny, z ).grad += m[strv1];
									hit++;
								}
									//printf("dsa\n");
								...
								#ifdef Times
									fprintf(fp,"%.32f %.32f %.32f\n", in( x, y, z ),grad_next_layer( i, j, k ),in( x, y, z ) * grad_next_layer( i, j, k ));
								#endif 
