...
int  init(int u ){
	...
	// cout << m["00111110000011101011"] << endl ;
	// cout << m1["00111110000011101011"] << endl ;
	fclose(fp);
	return 0;
}
...
int main(int argc, char const *argv[]){
	...
	for ( long ep = 0; ep < 1; )
	{
		...
		for ( case_t& t : cases )
		{
			...
			if ( ep % 10 == 0 ){
				cout<< "bits" << atoi(argv[1]) << "  case " << ep << "  err=" << amse/ic << endl;
				// break;		
				cout << hit << " " << mul << " hit rate " << float(hit) / float(mul) <<endl;	
				// break;
			}
